...
class historyController extends Controller
{
  ...
  public function index(Request $request)
  {
    ...
    return response()->json(['status'=>'ok','data'=>$historys],200);
    // echo json_encode();
...
public function store(Request $request){
  ...
  if (!$request->input('histitle') || !$request->input('hisdescription') || !$request->input('hisDateEvent')|| !$request->input('hisshortDescription') )
  {
  ...
  $newhistory=$this->history->create($request);

  // Más información sobre respuestas en http://jsonapi.org/format/
  // Devolvemos el código HTTP 201 Created – [Creada] Respuesta a un history que resulta en una creación. Debería ser combinado con un encabezado Location, apuntando a la ubicación del nuevo recurso.

  $response = Response::make(json_encode(['data'=>$newhistory]), 201)->header('Location', 'http://www.dominio.local/fabricantes/'.$newhistory->id)->header('Content-Type', 'application/json');
  return $response;
}
/**
...
public function update(Request $request, $id)
{
  ...
  $historys=$this->history->show($id)[0];
  //if (!$request->input('user_id') || !$request->input('historyitle') || !$request->input('poscontent') || !$request->input('posdescription') || !$request->input('posphoto') || !$request->input('category_id') || !$request->input('posshortdesc'))
  ...
  if ($request->method() === 'PATCH')
  {
    ...
    if ($histitle)
    {
      $historys->histitle = $histitle;
      $bandera=true;
    }

    if ($hisdescription)
    {
      $historys->hisdescription = $hisdescription;
      $bandera=true;
    }

    if ($hisDateEvent)
    {
      $historys->hisDateEvent = $hisDateEvent;
      $bandera=true;
    }

    if ($hisshortDescription)
    {
      $historys->hisshortDescription = $hisshortDescription;
      $bandera=true;
    }
  ...
  */
  ...
  if (!$histitle || !$hisdescription || !$hisDateEvent || !$hisshortDescription )
  {
    // Se devuelve un array errors con los errores encontrados y cabecera HTTP 422 Unprocessable Entity – [Entidad improcesable] Utilizada para errores de validación.
